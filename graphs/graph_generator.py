...
class GraphGenerator:
    ...
    Methods
    -------
    generate_random_graph(conn_component_count: int = 3, vertex_count: int = 15)\
            -> nx.Graph:
        Возвращает случайно сгенерированный граф, представляющий собой
        одну или несколько компонент связности без циклов.
    ...
    def generate_random_graph(
        ...
        """Возвращает случайно сгенерированный граф, представляющий собой
        одну или несколько компонент связности без циклов. Названия вершин уникальны
        и состоят из латинских букв. Вершины, относящиеся к одной компоненте,
        имеют одинаковый цвет.

        :param conn_component_count: Количество компонентов связности в графе.
        :param vertex_count:Количество вершин в графе.
        :raises TypeError: Если количество вершин или количество компонентов
        не являются целыми числами.
        :raises ValueError: Если количество вершин или количество компонентов
        меньше единицы, если количество вершин меньше чем количество компонентов.
        :return: Граф.
        """
    ...
    def show_plot(graph: nx.Graph) -> None:
        ...
        nx.draw_circular(graph, with_labels=True, node_color=color_map)
        plt.show()

    @staticmethod
    def __add_nodes_to_graph(
        graph: nx.DiGraph, vertices_by_components: list[list[str]], colors: list[str]
    ) -> None:
        raise NotImplementedError()  # Удалить при реализации метода

    @staticmethod
    def __add_edges_to_graph(
        graph: nx.DiGraph, vertices_by_components: list[list[str]]
    ) -> None:
        raise NotImplementedError()  # Удалить при реализации метода
...
if __name__ == "__main__":
    graph = GraphGenerator.generate_random_graph(2, 15)
